...
import Card from '../Cards/CollaborateCard';
import 'slick-carousel/slick/slick.css';
import 'slick-carousel/slick/slick-theme.css';

const CollaborateCarousel = ({ slides }) => {
  const settings = {
    dots: false,
    infinite: true,
    speed: 500,
    ...
    <Slider {...settings}>
      {slides.map((slide, index) => (
        <div key={index} className="carousel-card">
          <Card image={slide.image} title={slide.title} description={slide.description} register={slide.register} />
        </div>
      ))}
    </Slider>
  );
...
export default CollaborateCarousel;