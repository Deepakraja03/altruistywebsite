...
import Card from '../Cards/Jobs';
import 'slick-carousel/slick/slick.css';
import 'slick-carousel/slick/slick-theme.css';

const JobCarousel = ({ slides }) => {
  const settings = {
    dots: true,
    infinite: true,
    speed: 500,
    ...
    <Slider {...settings}>
      {slides.map((slide, index) => (
        <div key={index} className="carousel-card">
          <Card image={slide.image} title={slide.title} location={slide.location} salary={slide.salary} type={slide.type} description={slide.description} register={slide.register} />
        </div>
      ))}
    </Slider>
  );
